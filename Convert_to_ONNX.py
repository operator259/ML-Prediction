...
categories_json = {
    "Home Team": categories[0].tolist(),
    "Away Team": categories[1].tolist(),
    "Venue": categories[2].tolist()
}
...
onnx_model_home = convert_sklearn(best_model_home, initial_types=input_type)
with open('model_home.onnx', 'wb') as f:
    f.write(onnx_model_home.SerializeToString())

onnx_model_away = convert_sklearn(best_model_away, initial_types=input_type)
with open('model_away.onnx', 'wb') as f:
    f.write(onnx_model_away.SerializeToString())
